...
Read homepage, services, service detail, company information, and testimonials — Public App Router pages and server reads return public content only.

Submit contact/enquiry — Public /contact#request-quote Server Action. Zod validation, honeypot, optional published Service, NEW unassigned Lead, and safe response; no account required.

Sign in/out — Staff use the Auth.js credentials handler and /admin/login UI. Email/password authenticate against User; there is no public self-registration. Real database flow remains unverified here.

List/search/filter leads; read detail — STAFF and ADMIN use /admin/leads and /admin/leads/[id] server reads. A fresh session/User check is required; all leads are visible in this initial workflow. An assigned-only view remains planned.

Read dashboard metrics — STAFF and ADMIN use the protected /admin server read. It returns all-lead status/current-UTC-month counts, five recent Leads, six UTC months, and up to five Services ranked by linked enquiries. There is no public endpoint.

Update status; add internal note — STAFF and ADMIN use lead-detail Server Actions. The server verifies current staff identity, validates status/content, and derives note author from the session.

Assign/unassign lead — ADMIN uses a lead-detail Server Action. The server checks ADMIN; only ACTIVE Users are eligible for new assignment. Historical assignments to disabled accounts remain visible.

List/create/edit/publish/unpublish Services — ADMIN uses /admin/services pages and Server Actions. Every read/write requires a fresh ADMIN check and Zod validation; there is no hard deletion.

List/read Services — Public /services and /services/[slug] server reads return only published = true records; unknown or unpublished detail returns 404.

List/create/edit/publish/unpublish Testimonials — ADMIN uses /admin/testimonials pages and Server Actions. Every read/write requires a fresh ADMIN check and Zod validation; there is no hard deletion.

Read Testimonials — The public homepage server read returns only published records, displayed as fictional portfolio examples.

Read/update SiteSettings — ADMIN uses the /admin/settings page and Server Action. The server checks ADMIN, validates all four fields, and upserts only singleton ID 1.

Read public business settings — Public shared layout and page reads return name/contact fields only. A missing row uses NexaService with no contact details; public reads never write.

List/create/edit/disable/reactivate staff — ADMIN uses /admin/users, /admin/users/new, /admin/users/[id]/edit, and Server Actions. Every read/write requires a fresh ACTIVE ADMIN check, Zod validation, bcrypt hashes for new passwords, and serialized safeguards. There is no public registration or password-edit interface.
