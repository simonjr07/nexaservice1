...
export const metadata: Metadata = {
  title: "Services",
  description: "Explore practical commercial workplace and facility services from NexaService.",
  openGraph: {
    ...publicOpenGraph,
    title: "NexaService services",
    description: "Explore practical commercial workplace and facility services from NexaService.",
  },
};
  ...
  return <>
    <section className="bg-deep px-5 py-20 text-white sm:px-8 md:py-28 lg:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <p className="mt-7 max-w-2xl text-base leading-8 text-white/70">Explore how {settings.businessName} can support the upkeep and day-to-day needs of a commercial workplace. Each request begins with a conversation about your site.</p>
      </div>
    </section>
    <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12" aria-label="Available services">
      {services.length === 0 ? <div className="rounded-3xl border border-line bg-white p-10"><h2 className="text-2xl font-semibold">Services are being prepared</h2><p className="mt-3 text-sm leading-7 text-muted">No service details are published yet. Tell us about your workplace and the support you need.</p><Link href="/contact#request-quote" className="mt-6 inline-flex min-h-11 items-center rounded-full bg-deep px-6 text-sm font-semibold text-white hover:bg-sea">Request a quote ↗</Link></div> :
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{services.map((service, index) => <article key={service.id} className="flex flex-col rounded-3xl border border-line bg-white p-8 transition-[border-color,transform] hover:border-sea/35 motion-safe:hover:-translate-y-0.5">
      ...
      <div className="mt-12 flex flex-col gap-6 rounded-3xl bg-[#eaf0e8] p-8 sm:flex-row sm:items-center sm:justify-between sm:p-10">
        <div><h2 className="text-2xl font-semibold tracking-[-0.04em]">Not sure where your need fits?</h2><p className="mt-2 text-sm leading-7 text-muted">Describe the site or facility issue and we can discuss the next step.</p></div>
