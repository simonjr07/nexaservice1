...
export default async function AdminServicesPage() {
  ...
  return <>
    ...
    {services.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed border-[#c7d5cf] bg-white p-10"><h2 className="text-xl font-semibold">No services yet</h2><p className="mt-2 text-sm text-muted">Create a draft to begin building your public service listing.</p></div> :
      <div className="mt-8 overflow-x-auto rounded-2xl border border-line bg-white"><table className="min-w-[760px] w-full border-collapse text-left text-sm">
        ...
        <tbody className="divide-y divide-line">{services.map((service) => <tr key={service.id} className="align-top">
          ...
          <td className="px-5 py-4 text-muted">/{service.slug}</td>
          <td className="px-5 py-4"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${service.published ? "bg-[#eaf0e8] text-sea" : "bg-[#f0f2f0] text-muted"}`}>{service.published ? "Published" : "Draft"}</span></td>
          <td className="px-5 py-4 text-xs leading-6 text-muted"><time dateTime={service.createdAt.toISOString()}>{service.createdAt.toLocaleDateString("en-GB")}</time><br /><time dateTime={service.updatedAt.toISOString()}>{service.updatedAt.toLocaleDateString("en-GB")}</time></td>
          <td className="px-5 py-4"><div className="flex items-center gap-4"><Link href={`/admin/services/${service.id}/edit`} className="text-sm font-semibold text-sea hover:underline">Edit</Link><ServicePublicationForm published={service.published} action={setServicePublicationAction.bind(null, service.id, !service.published)} /></div></td>
        </tr>)}</tbody>
      </table></div>}
  </>;
}
