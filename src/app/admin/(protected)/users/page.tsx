...
export default async function StaffPage() {
  ...
  return <>
    ...
    {accounts.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed border-[#c7d5cf] bg-white p-10"><h2 className="text-xl font-semibold">No staff accounts</h2><p className="mt-2 text-sm text-muted">Create an internal account to grant dashboard access.</p><Link href="/admin/users/new" className="mt-5 inline-flex min-h-11 items-center text-sm font-semibold text-sea hover:underline">Create a staff account ↗</Link></div> :
      <div className="mt-8 overflow-x-auto rounded-2xl border border-line bg-white"><table className="admin-records-table w-full border-collapse text-left text-sm">
        ...
        <tbody className="divide-y divide-line">{accounts.map((person) => <tr key={person.id} className="align-top">
          ...
          <td data-label="Role" className="px-5 py-4">{person.role}</td>
          <td data-label="Status" className="px-5 py-4"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${person.status === "ACTIVE" ? "bg-[#eaf0e8] text-sea" : "bg-[#f0f2f0] text-muted"}`}>{person.status === "ACTIVE" ? "Active" : "Disabled"}</span></td>
          <td data-label="Created" className="px-5 py-4 text-muted"><time dateTime={person.createdAt.toISOString()}>{person.createdAt.toLocaleDateString("en-GB")}</time></td>
          <td data-label="Actions" className="px-5 py-4"><div className="flex flex-wrap items-start gap-4"><Link href={`/admin/users/${person.id}/edit`} className="text-sm font-semibold text-sea hover:underline">Edit</Link><StaffStatusForm name={person.name} status={person.status} self={person.id === admin.id} action={setStaffStatusAction.bind(null, person.id, person.status === "ACTIVE" ? "DISABLED" : "ACTIVE")} /></div></td>
        </tr>)}</tbody>
      </table></div>}
  </>;
}
