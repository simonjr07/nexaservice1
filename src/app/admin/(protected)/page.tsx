    ...
    <>
      ...
      <section className="relative mt-10 overflow-hidden rounded-[2rem] bg-deep p-8 text-white sm:p-10" aria-labelledby="preview-heading">
        ...
        <div className="relative max-w-xl">
          ...
          <p className="mt-5 text-sm leading-7 text-white/70">The lead inbox shows real enquiries, and administrators can manage public services. Other management areas are planned for later work.</p>
        </div>
      </section>
      <section className="mt-12" aria-labelledby="modules-heading">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><h2 id="modules-heading" className="text-2xl font-semibold tracking-[-0.04em]">Workspace areas</h2><p className="text-sm text-muted">Leads and Services are available; other areas are previews.</p></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {modules.filter((module) => !module.adminOnly || user.role === "ADMIN").map((module) => (
            <Link key={module.href} href={module.href} className="group flex min-h-56 flex-col rounded-2xl border border-line bg-white p-6 transition-colors hover:border-sea/40">
              <div className="flex items-start justify-between"><span className="text-xs font-semibold tracking-[0.15em] text-sea">{module.number} / {module.title === "Leads" || module.title === "Services" ? "AVAILABLE" : "PLANNED"}</span><span aria-hidden="true" className="text-lg text-sea transition-transform group-hover:-translate-y-1 group-hover:translate-x-1">↗</span></div>
