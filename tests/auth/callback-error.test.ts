...
const mockedAllowRequest = vi.mocked(allowRequest);
...
describe("Auth.js credentials callback errors", () => {
  ...
  it("returns the same generic callback failure when login attempts are limited", async () => {
    mockedAllowRequest.mockResolvedValueOnce(false);
    mockedAuthenticate.mockClear();
    const options = { ...authOptions, secret: "test-only-auth-secret-with-sufficient-length" };
    const csrf = await AuthHandler({
      options,
      req: { action: "csrf", method: "GET", headers: { host: "localhost:3000" }, cookies: {}, query: {} } as never,
    ...
    const callback = await AuthHandler({
      options,
      req: {
        action: "callback", providerId: "credentials", method: "POST", headers: { host: "localhost:3000" },
        cookies: { [csrfCookie!.name]: csrfCookie!.value },
        body: { email: "staff@example.test", password: "test-only-password", csrfToken, json: "true" }, query: {},
      } as never,
    });
    expect(callback.status).toBe(401);
    expect(new URL(callback.redirect!).searchParams.get("error")).toBe("CredentialsSignin");
